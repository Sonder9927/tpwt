...
def sensitivity_TPWT(
    ...
    # create sac
    sac_file = f"{per}.sac"
    tpwt.lib.sac_sens_new_period(sac_file, per)
    ...
    tpwt.lib.sac_sens(input, vel, smooth, sen_output)
...
def rdsetupsimul_phamp_from_earthquake_TPWT(
    ...
    rdsetupsimul_phamp = pather.binuse(
...
def sort_files_for_iter(per, nodes, staid, sens_dat, out_dir):
    ...
    pather.remove_targets([f"resmax{per}.dat", f"phampcor.v3.{per}"])
...
def simanner_TPWT_run(eqlistper, kern, bin_from="./"):
    ...
    """
    simanner = pather.binuse(f"simannerr{kern}.kern", bin_from=bin_from)
...
def find_bad_kern(
    ...
    *,
    ...
    """
    fn = pather.binuse(f"find_bad_kern{kern}", bin_from=bin_from)
    c_string = f"{fn} "
    if kern == 300:
...
def simanner_and_gridgenvar(kern, per, smooth, damp, region, eq, bin_from="./"):
    ...
    # create grid
    gridgenvar = pather.binuse("gridgenvar.yang_v2", bin_from=bin_from)
